...
function openPlayer(path, options) {
  const player = omxp_spawn.player(path, options);
  player.once("close", function (silent) {
    if (typeof tick !== "undefined") clearInterval(tick);
    if (silent) {
      // eventEmitter.emit("error", url);
      throw new Error("error play video");
    } else {
      eventEmitter.emit("finish", path);
    }
  });
  var conn_interval = setInterval(function () {
    if (fs.existsSync("/tmp/omxplayerdbus." + process.env.USER)) {
      bus = dbus.sessionBus({
        busAddress: fs
          .readFileSync("/tmp/omxplayerdbus." + process.env.USER, "ascii")
          .trim(),
      });
  ...
  // return { path, pid: player.pid };
  return player;
}

function quitPlayer(signal) {
  signal.stdin.write("q");
}
...
function propertyRead(prop, cb) {
  if (
    typeof omx_methods.properties[prop] !== "undefined" &&
    omx_methods.properties[prop].read
  ) {
      ...
      bus.invoke(
        {
          ...
          destination: "org.mpris.MediaPlayer20.omxplayer",
          member: action,
          signature: omx_methods.methods[action].write
            ? omx_methods.methods[action].signature
            : null,
...
function setVolume(val, cb) {
  ...
  exec(cmd + val, function (stderr, stdout) {
    ...
    var vol_d = parseFloat(
      stdout.substr(stdout.indexOf("double") + 7, stdout.length)
    );
    cb(null, vol_d);
  });
}
...
module.exports.quitPlayer = quitPlayer;
